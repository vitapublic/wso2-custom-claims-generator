...
public class CustomClaimsRetriever implements ClaimsRetriever {
    ...
    /**
     * You need to implement getClaims method and use accessToken there as parameter. In getClaims method you can implement
     * caching keys, logic etc. And this method should call from CustomTokenGenerator
     * @param endUserName
     * @param accessToken
     * @return
     * @throws APIManagementException
     */
    public SortedMap<String, String> getClaims(String endUserName, String accessToken) throws APIManagementException {
        SortedMap<String, String> claimValues;
        try {
            int tenantId = APIUtil.getTenantId(endUserName);
            //check in local cache
            String key = endUserName + ":" + tenantId + ":"+accessToken;
            ClaimCacheKey cacheKey = new ClaimCacheKey(key);
            //Object result = claimsLocalCache.getValueFromCache(cacheKey);
            Object result = claimsLocalCache.get(cacheKey);
            if (result != null) {
